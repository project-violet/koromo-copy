...
namespace Koromo_Copy.Hitomi
{
    public class HitomiParser
    {
        ...
        static public HitomiArticle ParseGalleryBlock(string source)
        {
            ...
            article.Artists = nodes.SelectNodes(".//div[@class='artist-list']//li").Select(node => node.SelectSingleNode("./a").InnerText).ToArray();

            var contents = nodes.SelectSingleNode("./div[2]/table");
            article.Series = contents.SelectNodes("./tr[1]/td[2]/ul/li").Select(node => node.SelectSingleNode(".//a").InnerText).ToArray();
